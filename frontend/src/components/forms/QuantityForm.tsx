...
export const QuantityForm: React.FC<Props> = ({ onSuccess }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('QuantityForm:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-3 gap-4 items-end">
      <div>
        ...
        <select value={form.item} onChange={e => setForm(p => ({ ...p, item: e.target.value }))} required className={INPUT}>
          <option value="" disabled>Select item…</option>
          <option value="EVA">EVA</option>
          <option value="EVA for SC">EVA for SC</option>
          <option value="EVA for DC">EVA for DC</option>
        </select>
      </div>
      <div>
