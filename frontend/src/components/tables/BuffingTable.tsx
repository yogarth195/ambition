...
import { buffingApi } from '@/api/buffing';
...
export const BuffingTable: React.FC<Props> = ({ refreshKey }) => {
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      try {
        const res = await buffingApi.getAll(month ? { monthBelongs: month } : undefined);
        setData(res.data ?? []);
