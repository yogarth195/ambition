...
interface Row { id: string; amount: number; entryDate: string; monthBelongs: string; }
...
export const LabourTable: React.FC<Props> = ({ refreshKey }) => {
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      try {
        const res = await labourApi.getAll(month ? { monthBelongs: month } : undefined);
        setData(res.data ?? []);
  ...
  const columns = [
    ...
    { header: 'Amount (₹)', render: (r: Row) => <span className="text-sm font-medium text-gray-900 tabular-nums">₹{r.amount.toLocaleString('en-IN')}</span> },
