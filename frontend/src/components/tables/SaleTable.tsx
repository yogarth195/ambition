...
import { DataTable, DateRangeFilter } from '@/components/ui/DataTable';

interface Row { id: string; item: string; value: number; entryDate: string; }
...
export const SaleTable: React.FC<Props> = ({ refreshKey }) => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        const res = await saleApi.getAll(Object.keys(params).length ? params : undefined);
        setData(res.data ?? []);
